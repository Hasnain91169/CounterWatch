...
def score_candidate(cand, state, data, M, pair_lookup):
    ...
    for e in state["enemy"]:
        ...
        counter   += M.get(cand, {}).get(e, 0.0) * weight   # you beat them
        countered += M.get(e, {}).get(cand, 0.0) * weight   # they beat you

    team = list(state.get("my_team", {}).values())
    synergy = synergy_score(cand, team, data, pair_lookup)
    ...
    return {
        "hero": heroes[cand]["name"], "slug": cand, "total": round(total, 2),
        "counter": round(counter, 2), "countered": round(countered, 2),
        "synergy": round(synergy, 2), "comfort": comfort,
